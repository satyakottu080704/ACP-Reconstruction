...
def _apply_same_crop(raw_image: np.ndarray, clahe_image: np.ndarray, cropped: np.ndarray) -> np.ndarray:
    ...
    def _score(candidate: np.ndarray) -> float:
        if candidate.shape[:2] != cropped.shape[:2]:
            return float("inf")
        candidate_gray = cv2.cvtColor(candidate, cv2.COLOR_BGR2GRAY) if len(candidate.shape) == 3 else candidate
        cropped_gray = cv2.cvtColor(cropped, cv2.COLOR_BGR2GRAY) if len(cropped.shape) == 3 else cropped
        return float(np.mean(np.abs(candidate_gray.astype(np.int16) - cropped_gray.astype(np.int16))))
    ...
    if dx > 0 and dy <= 10:
        ...
        candidates = [
            raw_image[:, dx:][:crop_h, :crop_w],
            raw_image[:, :crop_w][:crop_h, :crop_w],
        ]
        return min(candidates, key=_score)
    elif dy > 0 and dx <= 10:
        # Vertical crop — form on top or bottom
        candidates = [
            raw_image[dy:, :][:crop_h, :crop_w],
            raw_image[:crop_h, :][:crop_h, :crop_w],
        ]
        return min(candidates, key=_score)
    else:
...
def detect_and_crop_form(
    ...
    if green_left > green_right * 2 and green_left > 1000:
        print("[PREPROCESS] Form/logo evidence detected on LEFT, overriding density heuristic")
        forced_left = True
    elif green_right > green_left * 2 and green_right > 1000:
        print("[PREPROCESS] Form/logo evidence detected on RIGHT, overriding density heuristic")
        forced_right = True
    elif left_avg > right_avg * 1.2 and left_avg > 3:
    ...
    if forced_left:
        ...
        if cut_x <= 0:
            cut_x = _find_side_form_boundary(image, side="left")
        if cut_x > 0:
    ...
    if forced_right:
        ...
        if cut_x_from_right <= 0:
            cut_x_from_right = w - _find_side_form_boundary(image, side="right")
        if cut_x_from_right > 0:
...
def _find_side_form_boundary(image: np.ndarray, side: str = "left") -> int:
    """Find a side form boundary when the sketch is denser than the form.

    Some photos have a quiet form panel and a heavily annotated sketch. A
    density-drop test then fails because density increases after the form. In
    that case, use the strong vertical border between the form and graph paper.
    Returns an x coordinate from the left edge.
    """
    h, w = image.shape[:2]
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    edges = cv2.Canny(gray, 50, 150)
    col_strength = edges.mean(axis=0).astype(np.float32)
    kernel = max(9, w // 250)
    if kernel % 2 == 0:
        kernel += 1
    col_strength = cv2.GaussianBlur(col_strength.reshape(1, -1), (kernel, 1), 0).ravel()
    ...
    window = col_strength[start:end]
    if window.size == 0:
        return 0
    threshold = max(2.0, float(np.mean(col_strength)) * 0.45)
    above = np.where(window > threshold)[0] + start
    if above.size == 0:
        return 0
    groups = []
    current = [int(above[0])]
    for x_val in above[1:]:
        x_int = int(x_val)
        if x_int == current[-1] + 1:
            current.append(x_int)
        else:
    ...
    if not groups:
        return 0
    group = groups[-1] if side == "right" else groups[0]
    best = max(group, key=lambda x_val: col_strength[x_val])
    print(f"[PREPROCESS] Form boundary fallback on {side.upper()} at x={best}")
    return best
