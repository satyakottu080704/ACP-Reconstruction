...
Schema:
{
  ...
  "rooms":   [{"name":str,"x":int,"y":int,"w":int,"h":int,"type":"clear"|"acm"|"no_access"}],
...
- "rooms": (x,y) is the CENTRE of the room where its name label belongs.
  - "w" and "h" are the estimated proportional width and height of the room's boundary in the 1000x1000 coordinate space.
  - "type" MUST be "acm" if the room has diagonal hatching, red patterns, or red markings, "no_access" if it has a large crossed X or "no access" labels, and "clear" otherwise.
  - Use the room name written on the sketch; if none, use a sensible type (e.g. "Bedroom", "Kitchen", "Bathroom", "Hall", "Landing").
...
def _normalize_layout(raw: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for r in raw.get("rooms") or []:
        ...
        if not norm_name:
            continue
        r_type = str(r.get("type", "clear")).strip().lower()
        if r_type not in ("clear", "acm", "no_access"):
            r_type = "clear"
        room_dict = {
            "name": norm_name,
            "x": _clamp(_num(r.get("x"))),
            "y": _clamp(_num(r.get("y"))),
            "w": _clamp(_num(r.get("w"), 100.0)),
            "h": _clamp(_num(r.get("h"), 100.0)),
            "type": r_type,
        }
        if num:
...
def extract_floor_plan_layout(image_path: str) -> Dict[str, Any]:
    """Extract a vector floor-plan layout from a sketch image via the configured OpenAI vision model.
