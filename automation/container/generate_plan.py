...
try:
    ...
    try:
        # Repo layout (no src/ prefix) â€” e.g. local test runs.
...
def _is_renderable_floor(fl: Dict[str, Any]) -> bool:
    ...
    fitted to the page â€” ``_fit_layout_to_canvas`` leaves a zero-span layout
...
def generate_vsdx_natively(layout: Dict[str, Any], vsdx_path: Path):
    ...
    # aborting the whole render â€” so fall back to one combined page instead.
...
def draw_svg_plan(layout: Dict[str, Any], output_path: Path):
    ...
    # Plain white canvas (no grid â€” keeps it reading as a finished drawing).
...
def delegate_drawing_to_windows_agent(image_path: Path, project_number: str, vsdx_path: Path) -> bool:
    ...
    output_folder = os.environ.get(
        "SHAREPOINT_OUTPUT_FOLDER", "General/AI Automation/AI_COMPLETED"
    ).strip().strip("/")
    ...
    if mode == "http":
        ...
        # 2. Poll 'General/AI Automation/AI_COMPLETED' for the VSDX file
...
def _write_quality_sidecar(layout: Dict[str, Any], layout_is_real: bool, vsdx_path: Path) -> None:
    ...
    STRUCTURAL sanity only â€” it proves the plan is not broken/empty/placeholder.
